...
Example Code	c void funcA(int n) { if (n == 0) return; printf("%d ", n); funcA(n - 1); }	c void funcA(int n) { if (n <= 0) return; printf("%d ", n); funcB(n - 1); } void funcB(int n) { if (n <= 0) return; printf("%d ", n); funcA(n - 2); }
